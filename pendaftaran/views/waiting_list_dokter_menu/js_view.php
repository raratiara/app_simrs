...
$("#addcarow").on("click", function () { 
	var countRow = document.getElementById("hdnttlRow").value; 
	var nextRow = countRow++; 
	
	var newRowContent = '<tr><th scope="col">2</th><th scope="col"><input type="text" id="txtkode" name="txtkode[]" /></th><th scope="col"><input type="text" id="txtnama" name="txtnama[]" readonly style="background-color: #d3d4d4;"/></th><th scope="col" style="text-align:right;"><input type="text" id="txtqty" name="txtqty[]" /></th><th scope="col"><input type="text" id="txtsatuantxtsatuan" name="txtsatuan[]" style="background-color: #d3d4d4;" readonly/><input type="hidden" id="hdnharga" name="hdnharga[]"></th><th scope="col"><input type="button" class="btn btn-default red btn-outline btn-circle btn-sm trash" id="addcarow" value="Delete Row" /> </th></tr>';

	$("#tblResep tbody").append(newRowContent);
	$('[id="hdnttlRow"]').val(nextRow);
    ...
    $.ajax({
        ...
        { 
			if(data != false){ console.log(data);
				$('[id="txtnama1"]').val(data[0]['nama']);
				$('[id="txtsatuann1"]').val(data[0]['satuan_id']);
			} else { 
				title = '<div class="text-center" style="padding-top:20px;padding-bottom:10px;"><i class="fa fa-exclamation-circle fa-5x" style="color:red"></i></div>';
				btn = '<br/><button class="btn blue" data-dismiss="modal">OK</button>';
				msg = '<p>Gagal peroleh data.</p>';
