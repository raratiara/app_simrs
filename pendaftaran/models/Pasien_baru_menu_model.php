...
class Pasien_baru_menu_model extends MY_Model
{
	...
	public function getNextNumber() { 
		...
		$yearcode = date('y');
		$monthcode = date('m');
		$dateee = $yearcode.$monthcode;

		$cek = $this->db->query("select * from pasien where SUBSTRING(no_rekam_medis, 3, 4) = '2502'");
		$rs_cek = $cek->result_array();

		if(empty($rs_cek)){
			$num = '0001';
		}else{
			$cek2 = $this->db->query("select max(no_rekam_medis) as maxnum from pasien where SUBSTRING(no_rekam_medis, 3, 4) = '2502'");
