...
class Waiting_list_farmasi_menu_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "pendaftaran/waiting_list_farmasi_menu_model";
	...
	public function edit_data($post) { 
		if(!empty($post['id_pendaftaran'])){

			if(trim($post['status_farmasi']) == 4){ 
				$data_pendaftaran = [
					'status' => 5,
					'status_farmasi'		=> trim($post['status_farmasi'])
				];
			}else{ 
				$data_pendaftaran = [
					'status_farmasi'		=> trim($post['status_farmasi'])
			...
			return  $rs = $this->db->update('pendaftaran', $data_pendaftaran, [$this->primary_key => trim($post['id_pendaftaran'])]);
	...
	public function getRowData($id) { 
		$mTable = '(select a.id as id_pendaftaran, a.date_pendaftaran, a.no_urut, a.poli_id, a.dokter_id, a.jenis_pembayaran, a.status, a.created_by, a.created_at, b.nama as poli, c.nama as dokter, d.nama as pasien, 		  e.nama as status_name, d.alamat_tinggal, d.no_tinggal, d.rt_tinggal, d.rw_tinggal, 				d.provinsi_id_tinggal, d.kabkota_id_tinggal, d.kec_id_tinggal, d.kel_id_tinggal, 				d.no_rekam_medis, a.date_jadwal_pemeriksaan, a.jam_jadwal_pemeriksaan
					...
					,k.cara_pembayaran, k.date_pembayaran, k.total_pembayaran, a.status_farmasi, l.nama as status_farmasi_desc
                    from pendaftaran a
                    ...
                    left join status_farmasi l on l.id = a.status_farmasi
				)dt';
	...
	{
		$sql = '
	   			select a.no_urut, d.no_rekam_medis, d.nama as pasien, b.nama as poli, c.nama as dokter, e.nama as status_name
                    from pendaftaran a
					left join poli b on b.id = a.poli_id
					left join dokter c on c.id = a.dokter_id
					left join pasien d on d.id = a.pasien_id
