...
class Waiting_list_kasir_menu_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "pendaftaran/waiting_list_kasir_menu_model";
 	protected $table_name 			= _PREFIX_TABLE."pembayaran";
	...
	public function edit_data($post) { 
		if(!empty($post['id'])){
			...
			$data = [
				'metode_pembayaran'		=> trim($post['metode_pembayaran'])
			];

			if(trim($post['status']) == 2){
				$data_pendaftaran = [
					'status'		=> 4 //waiting farmasi
				];
				$this->db->update('pendaftaran', $data_pendaftaran, [$this->primary_key => trim($post['pendaftaran_id'])]);
			}
	...
	public function getRowData($id) { 
		$mTable = '(select a.id as id_pendaftaran, a.date_pendaftaran, a.no_urut, a.poli_id, a.dokter_id, a.jenis_pembayaran, a.status, a.created_by, a.created_at, b.nama as poli, c.nama as dokter, d.nama as pasien, 		  e.nama as status_name, d.alamat_tinggal, d.no_tinggal, d.rt_tinggal, d.rw_tinggal, 				d.provinsi_id_tinggal, d.kabkota_id_tinggal, d.kec_id_tinggal, d.kel_id_tinggal, 				d.no_rekam_medis, a.date_jadwal_pemeriksaan, a.jam_jadwal_pemeriksaan
					...
					j.tinggi_badan, j.suhu_tubuh, j.tekanan_darah, j.saturasi, j.berat_badan, j.denyut_nadi, j.frekuensi_napas, j.tingkat_nyeri, d.jenis_kelamin
					,k.cara_pembayaran, k.date_pembayaran, k.total_pembayaran, k.invoice_no, k.invoice_date,
					if(d.jenis_kelamin = "P","Perempuan","Laki-Laki") as jenis_kelamin_desc,
					sum(l.nominal) as biaya_obat, count(l.id) as qty_item_obat,
					m.tarif as biaya_dokter, (sum(l.nominal)+m.tarif) as ttl_biaya, "0" as diskon
                    from pendaftaran a
                    ...
                    left join pembayaran k on k.pendaftaran_id = a.id
                    left join resep l on l.diagnosa_id = j.id
                    left join tarif_dokter m on m.dokter_id = a.dokter_id
				)dt';
