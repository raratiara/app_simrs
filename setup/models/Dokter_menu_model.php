...
class Dokter_menu_model extends MY_Model
{
	/* Module */
 	...
 	/* upload */
 	protected $attachment_folder	= "./uploads/data/dokter";
	...
	public function upload_file($id = "", $fieldname= "", $replace=FALSE, $oldfilename= "", $array=FALSE, $i=0) { 
		...
		if(!empty($id) && !empty($fieldname)){ 
			...
			if($array){ 
				...
				$_FILES['foto']['name'] = $_FILES[$fieldname]['name'];
				$_FILES['foto']['type'] = $_FILES[$fieldname]['type'];
				$_FILES['foto']['tmp_name'] = $_FILES[$fieldname]['tmp_name'];
				$_FILES['foto']['error'] = $_FILES[$fieldname]['error'];
				$_FILES['foto']['size'] = $_FILES[$fieldname]['size']; 
				// override field
				$fieldname = 'foto';
			}
			...
			{ 
				...
				//   ./uploads/data/dokter/1/
				if(!$this->upload->do_upload($fieldname)){  //echo 'aa'; die();
	...
	public function getExpensesRows($id,$view,$print=FALSE){ 
		...
		if(!empty($rd)){ 
			...
			/*if($view){
				$arrSat = json_decode(json_encode($msHari), true);
				$arrS = [];
